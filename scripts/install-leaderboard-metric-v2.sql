-- Targeted production migration for the route-based leaderboard calculation.
-- Existing version-1 snapshots remain stored but are hidden by current clients
-- until their owners explicitly publish corrected values.
begin;
...
begin
  ...
  if p_calculation_version <> 2 then
